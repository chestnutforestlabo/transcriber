...
- イベント行では**併記チップ**(併記: 話題提示 / 併記: 質問)と**タグチップ**(周囲の話題)をクリックで付け外しできる
  - スキーム表のルール: 質問が新話題を開く場合は「話題提示」を併記、**AI情報の共有には話題提示か質問の併記が必須**(未併記だと赤字で「併記必須」と警告)
...
反映ルール: **✗要修正の項目は集計から除外**、ラベル・時刻の修正値と手動追加行はそのまま反映、各録音に「レビュー済み n 件・要修正除外 m 件・手動追加 k 件」が注記される。イベント件数表には「周囲の話題(タグ)」に加えて「併記: 話題提示 / 併記: 質問」(attrs.co_labels の集計)の行が出る。
