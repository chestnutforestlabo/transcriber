...
- **参加者アーカイブへ退避する**(参加者横断の条件比較に使う。コーディングが1件でも成功した場合は必須):
  - 参加者IDは、プロンプト末尾に「参加者ID: <ID>」の指定があればそれ、なければ**セッションフォルダのbasename**を使う
  - `rm -rf outputs/participants/<ID> && mkdir -p outputs/participants && cp -r outputs/coding outputs/participants/<ID>`
    (同じ参加者の再実行は旧アーカイブを置き換える)
  - 最終報告にアーカイブ先パスを記載する
