def get_sd_model(args):
    ...
    if args.diarization_model_name == "community":
        from models.Diarization.pyannote_speaker_dialization_community.model import (
            SpeechDiarization,
        )

        sd_model = SpeechDiarization(args)
    # Pyannote 3.1 + CALLHOME Japanese fine-tuned segmentation
    elif args.diarization_model_name == "pyannote_ja":
        from models.Diarization.pyannote_speaker_dialization_ja.model import (
            SpeechDiarization,
        )

        sd_model = SpeechDiarization(args)
    # BUT-FIT DiariZen WavLM-Large
    elif args.diarization_model_name == "diarizen":
        from models.Diarization.BUT_DiariZen.model import SpeechDiarization

        sd_model = SpeechDiarization(args)
    # Pyannote/speaker-dialization-precision-2
    elif args.diarization_model_name == "precision":
        raise NotImplementedError(
            "The precision diarization backend is not implemented."
        )
    else:
        raise ValueError(
            f"Unsupported diarization model: {args.diarization_model_name}"
        )
    return sd_model
...
def get_asr_model(args):
    ...
    if args.asr_model_name == "openai":
        from models.Recognition.OpenAI_Whisper_large_v3.model import (
    ...
    # Kotoba-Whisper-v2
        ...
        from models.Recognition.KotobaTech_Kotoba_whisper_v2.model import (
            AutomaticSpeechRecognition,
        )
