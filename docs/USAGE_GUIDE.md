...
- **重要**: 録音ファイルを `調査N` にリネームしてセッションフォルダへ置くときは、**sidecar も同じ名前(`調査N.time.json`)にリネーム**する(忘れると内容照合フォールバックに落ちて精度が±1秒程度になる)
...
**フォルダ名は参加者IDになる**(完了時に `outputs/participants/<フォルダ名>/` へ自動退避され、条件比較の集計単位になる)。本番は `P01` のような参加者名、ドライラン・練習は `_practice01` のように **`_` 始まり**にすると条件比較の集計から自動で外れる。

転送:

```bash
scp -r セッションフォルダ arata:/home/arata/sessions/P01
```
...
対象セッションフォルダ: /home/arata/sessions/P01"'
```

フォルダ名と別の参加者IDにしたい場合だけ、末尾にもう1行「参加者ID: P01」を足す。
...
- 画面は上から **タイムラインバー → 文字起こし → コーディングレビュー → 再生コントロール**(波形表示は廃止)
- タイムラインバーのレーン構成: **会話/無言**(青/グレー)、**Q&A/停止**(AI応答=緑、システム停止=ピンク。ログ連携がある録音のみ)、**話者別イベントレーン**(視覚障害者/同行者/実験者)
  - イベントの色はラベル種類(話題提示=紺、質問=水色、AI情報の共有=赤、周囲説明=緑、応答なし発話=灰、ガイド発話=黄土)。「誰から」はレーンで分かる
  - 時間が重なるイベントは**1つのピル内の色セグメント**として表示。ホバーで中身一覧、**クリックで音声・文字起こし・レビュー一覧の3つが該当箇所へジャンプ**。レーンの空白部クリックでもシーク
  - 領域の高さは**仕切りバーのドラッグ**で変更できる(タイムラインの太さ / 文字起こし↔レビューの配分。設定は記憶される)
- レビュー一覧の各行に **区間 / イベント** のバッジが付く。区間=時間帯のラベル(会話・無言など)、イベント=発話単位のラベル
